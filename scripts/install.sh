#!/bin/sh
# gt-telegram install script
# Usage: curl -fsSL https://raw.githubusercontent.com/leonletto/gt-telegram/main/scripts/install.sh | sh
#
...
macos_resign() {
  ...
  if [ "$(uname -s)" = "Darwin" ]; then
    ...
    # Binary is already signed with Developer ID and notarized via CI/CD
  fi
}
...
install_from_release() {
  ...
  archive_name="gt-telegram_${version_num}_${os}_${arch}.tar.gz"
  ...
  log "Downloading gt-telegram $version for $os/$arch..."
...
install_from_source() {
  ...
  git clone --depth 1 "https://github.com/$REPO.git" "$tmp_dir/gt-telegram"
  cd "$tmp_dir/gt-telegram"
...
main() {
  ...
  if install_from_release "$os" "$arch" "$VERSION"; then
    log ""
    log "  gt-telegram installed to $INSTALL_DIR/$BINARY_NAME"
    check_path
    check_duplicates
    log ""
    log "  Run 'gt-telegram --help' to get started."
    return 0
  fi
  ...
  if install_with_go; then
    log ""
    log "  gt-telegram installed via 'go install'"
    check_duplicates
    log ""
    log "  Run 'gt-telegram --help' to get started."
    return 0
  ...
  if install_from_source; then
    log ""
    log "  gt-telegram built from source and installed to $INSTALL_DIR/$BINARY_NAME"
    check_path
    check_duplicates
    log ""
    log "  Run 'gt-telegram --help' to get started."
